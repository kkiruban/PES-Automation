...
def logIn_As_BA
  if ENV['UserRole'].include? 'Sit'
    YAML.load_file("./config/Login.yml")["AdminSit"]
  elsif ENV['UserRole'].include? 'Prod'
    YAML.load_file("./config/Login.yml")["AdminProd"]
  else
    writeFailure "Fail to find credentials for user role - #{ENV['UserRole']}"
  end
...
def logIn_As_OpsMgr
  if ENV['UserRole'].include? 'Sit'
    YAML.load_file("./config/Login.yml")["OperationManagerSit"]
  elsif ENV['UserRole'].include? 'Prod'
    YAML.load_file("./config/Login.yml")["OperationManagerProd"]
  else
    writeFailure "Fail to find credentials for user role - #{ENV['UserRole']}"
  end
end
